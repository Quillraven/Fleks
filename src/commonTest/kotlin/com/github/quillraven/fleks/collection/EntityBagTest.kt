...
class EntityBagTest {
    ...
    private val testEntity1 = Entity(0, version = 0)
    private val testEntity2 = Entity(1, version = 0)
    ...
    fun addValueToBag() {
        ...
        bag += Entity(42, version = 0)

        assertTrue(bag.isNotEmpty())
        assertEquals(1, bag.size)
        assertTrue(Entity(42, version = 0) in bag)
    }

    @Test
    fun removeValueFromBag() {
        val bag = MutableEntityBag(2)
        val e1 = Entity(0, version = 0)
        val e2 = Entity(1, version = 0)
        bag += e1
        bag += e2
    ...
    fun clearAllValuesFromBag() {
        val bag = MutableEntityBag()
        bag += Entity(42, version = 0)
        bag += Entity(43, version = 0)

        bag.clear()

        assertEquals(0, bag.size)
        assertFalse { Entity(42, version = 0) in bag }
        assertFalse { Entity(43, version = 0) in bag }
    }
    ...
    fun addValueToBagWithInsufficientCapacity() {
        ...
        assertEquals(1, bag.size)
        assertEquals(Entity(42, version = 0), bag[0])
    ...
    fun executeActionForEachValueOfBag() {
        ...
        assertEquals(listOf(Entity(42, version = 0), Entity(43, version = 0)), valuesCalled)
    }
    ...
    fun sortValuesByNormalEntityComparisonWithSizeLessThan7() {
        val bag = MutableEntityBag()
        repeat(6) { bag += Entity(6 - it, version = 0) }

        bag.sort(compareEntity(configureWorld { }) { e1, e2 -> e1.id.compareTo(e2.id) })

        repeat(6) {
            assertEquals(Entity(it + 1, version = 0), bag[it])
        }
    }
    ...
    fun sortValuesByNormalEntityComparisonWithSizeLessThan50ButGreater7() {
        val bag = MutableEntityBag()
        repeat(8) { bag += Entity(8 - it, version = 0) }
    ...
    fun sortValuesByNormalEntityComparisonWithSizeGreater50() {
        val bag = MutableEntityBag()
        repeat(51) { bag += Entity(51 - it, version = 0) }
    ...
    fun testContainsAll() {
        ...
        assertFalse(testBag.containsAll(listOf(Entity(2, version = 0))))
        assertTrue(testBag.containsAll(listOf(Entity(1, version = 0))))
    ...
    fun testAssociateByKeyAndValueSelector() {
        val expected = mapOf(Entity(2, version = 0) to 2, Entity(3, version = 0) to 3)

        val actual = testBag.associateBy(
            { Entity(it.id + 2, version = 0) },
            { it.id + 2 }
        )
    ...
    @Test
    fun testAssociateTo() {
        val expected = mapOf(testEntity1 to 0, testEntity2 to 1, Entity(2, version = 0) to 2)
        val destination = mutableMapOf(Entity(2, version = 0) to 2)
    ...
    fun testAssociateByToKeySelector() {
        val expected = mapOf(0 to testEntity1, 1 to testEntity2, 2 to Entity(2, version = 0))
        val destination = mutableMapOf(2 to Entity(2, version = 0))
    ...
    fun testAssociateByToKeyAndValueSelector() {
        val expected = mapOf(Entity(2, version = 0) to 2, Entity(3, version = 0) to 3, Entity(4, version = 0) to 4)
        val destination = mutableMapOf(Entity(4, version = 0) to 4)
    ...
    @Test
    fun testFilterTo() {
        val testEntity3 = Entity(2, version = 0)
    ...
    @Test
    fun testGroupByTo() {
        val expected1 = mapOf(0 to bagOf(testEntity1), 1 to bagOf(testEntity2), 2 to bagOf(Entity(2, version = 0)))
        val expected2 = mapOf(0 to listOf(3), 1 to listOf(3), 2 to listOf(3))

        val actual = testBag.groupByTo(mutableMapOf(2 to bagOf(Entity(2, version = 0)))) { it.id }
